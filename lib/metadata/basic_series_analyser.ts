...
function computeStats(data: Line): SeriesStats {
  const ys = data.points.map(p => p.y);
  const minValue = min(ys);
  const maxValue = max(ys);
  return {
    min: {
      value: minValue,
      labels: getLabelsAtValue(data.points, minValue)
    },
    max: {
      value: maxValue,
      labels: getLabelsAtValue(data.points, maxValue)
    },
    range: range(data),
    mean: mean(ys),
    median: median(ys),
...
function variance(series: Line, i: number, j: number, stats: SeriesStats): number {
  ...
  const rSquaredValue = 1 - rSquared(dataArray, linearRegressionLine(linearRegression(dataArray)));
  ...
  const variance = data.length < 3 ? 0 : (rSquaredValue * weights[0] + averageSecondDifference * weights[1] + dCProportion * weights[2] + normalizedPoints * weights[3]) * data.yRange() / (4 * stats.range);
  return variance
}
